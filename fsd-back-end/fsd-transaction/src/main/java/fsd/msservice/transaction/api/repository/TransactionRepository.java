...
public interface TransactionRepository extends JpaRepository<TransactionEntity, String> {
	...
	@Query(value = "select t.id as id, td.image_url as imageUrl, td.product_name as title,"
			...
			+ "   from transaction t2, transaction_detail td2"
			+ "   where t2.buyer_id = :id and t2.status = '0' and t2.id = td2.transaction_id group by td2.transaction_id) min_product"
			+ " where t.id = td.transaction_id and t.status = '0' and t.buyer_id = :id"
			+ " and t.id = min_product.transaction_id and td.seq = min_product.min_seq", nativeQuery = true)
	List<BuyerTransactionVO> findAllByBuyer(@Param("id") String buyerId);
	...
	@Query(value = "select t.id as id, td.image_url as imageUrl, td.product_name as title,"
			...
			+ " (select transaction_id, count(td2.id) as productCount, min(td2.seq) as min_seq"
			+ "   from transaction t2, transaction_detail td2"
			+ "   where t2.buyer_id = :id and t2.status = '0' and DATE_FORMAT(t2.created_date,'%Y%m%d') >= :start and DATE_FORMAT(t2.created_date,'%Y%m%d') <= :end and t2.id = td2.transaction_id group by td2.transaction_id) min_product"
			+ " where t.buyer_id = :id and t.status = '0' and DATE_FORMAT(t.created_date,'%Y%m%d') >= :start and DATE_FORMAT(t.created_date,'%Y%m%d') <= :end and t.id = td.transaction_id"
			+ " and t.id = min_product.transaction_id and td.seq = min_product.min_seq", nativeQuery = true)
	List<BuyerTransactionVO> findAllUnderBuyerByPeriod(@Param("id") String buyerId, @Param("start") String start,
	...
	List<SellerTransactionVO> findAllBySeller(@Param("id") String sellerId);

	@Query(value = "select td.transaction_id as id, t.buyer_name as buyerName, t.email as email, t.mobile as mobile,"
			+ " td.product_id as productId, td.product_name as productName, td.price as price,"
			+ " td.stock_number as purchaseCount, td.total_amount as totalAmount, td.total_tax as totalTax, t.created_date as created"
			+ " from transaction_detail td, transaction t where td.seller_id = :id and td.transaction_id = t.id"
			+ " and DATE_FORMAT(t.created_date,'%Y%m%d') >= :start and DATE_FORMAT(t.created_date,'%Y%m%d') <= :end"
			+ " order by t.created_date asc", nativeQuery = true)
	List<SellerTransactionVO> findAllUnderSellerByPeriod(@Param("id") String sellerId, @Param("start") String start,
			@Param("end") String end);
